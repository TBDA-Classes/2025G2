...
::
   ...
   └── scripts/
       ...
       ├── etl_agg_sensor_stats.py   # Temperature statistics
       ├── etl_agg_utilization.py    # Machine utilization
       ...
       └── etl_daily_runner.py       # Orchestrates all ETLs
...
Running the Backend
-------------------
Step-by-step setup (from README):
...
* ``/alerts_daily_count`` – Alert counts by type
* ``/alerts_detail`` – Individual alert records
* ``/data_status`` – Available date range and record counts

See :doc:`frontend` for how the UI consumes these endpoints.
