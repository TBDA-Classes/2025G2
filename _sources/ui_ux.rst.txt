...
* Provide a clear overview of machine status at a glance.
* Allow the client to explore details by date and shift.
...
* A left sidebar with primary navigation:
    * ``Dashboard`` – overall machine status and key metrics.
...
The Dashboard is the default entry point. It is composed of four main
blocks.
...
Below the cards there is a horizontal timeline that shows the machine status over 24 hours.

* Colours are used consistently: green (RUN), yellow (IDLE),
  red (DOWN).
* A time-window slider lets the user zoom into 10-minute intervals
...
* A Temperature History line chart displaying the evolution of key
  temperature variables (for example spindle and motor temperatures)
  during the day. The DA team filtered the relevant variables using
  statistical queries (UQ1–UQ3) to remove noisy or useless signals.
...
* The backend team exposed those metrics through FastAPI endpoints
  and ensured that response times stayed within UX targets (first
  feedback within 3 seconds, full results within 10 seconds).
* The frontend team implemented the Figma prototype in Next.js
